...
- 作り直す画像は Action Match の8枚（put / get / bring / throw × 男の子・女の子）と、Item Match の女の子 `word-target-girl-v1.png.webp` の計9枚。
- `action-put-boy-v4.png.webp` は所有者が承認し、公開用画像へ差し替えた。旧v3の元PNGとWebPはMac上の `source-images/legacy/` に保管している。残り8枚は、所有者のiPhone実機確認と次の指示があるまで制作・差し替えしない。
- 顔立ち・髪型・髪色は基準画像に合わせる。Action Match では2コマ構成、区切り線の色、小道具の画風と大きさも8枚で統一する。元画像の動作が読めることを優先する。
