...
375×667・390×844・430×932 の3サイズで4場面を繰り返し、画面に描かれた枠の中央と端寄りの位置へドロップします。成功時は、サイズごとに各場面の完了数・不正解回数・枠外ドロップ回数が表示され、最後に次のように表示されます。

```text
PASS chromium 375×667: ホーム起動時 speak() 0件、通常プレイ中の自動回復 0件、非表示場面の読み上げ 0件。
  bathroom: 6/6完了、不正解2回、枠外3回
  breakfast: 11/11完了、不正解4回、枠外6回
  living: 11/11完了、不正解4回、枠外6回
  bedroom: 8/8完了、不正解3回、枠外4回
（390×844、430×932 でも同様の PASS が続く）
AUTOMATED TEST PASSED: 3 sizes × 4 scenes, center and near-edge drops
```
...
判定位置を目で確認するには、アプリのURL末尾に `?debug=1` を付けて開きます（すでに `?v=...` が付いているURLなら `&debug=1`）。届け先の青い点線は判定範囲そのものです。ドロップ直後の赤い点が判定に使ったアイテムの絵の中心、緑の点は正解として受け付けた位置を表します。診断ログの `drop verdict` にもアイテム中心のSVG座標と指のSVG座標を記録します。
